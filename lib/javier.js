 ...
 * a wizard to generate javascript code from vxml files.
 ...
 */
function Javier() {
	var __browser__ = { warning: new Function() };
	...
	this.setBrowser = function (objRef) {
		__browser__ = objRef;
	}
	
	this.parse = function (node,level) {
	    var result = "";
		var child;
		var list;
		var snst = "\n";
		...
		for(var i = 0; i < node.childNodes.length;i++) {
			...
			switch(child.nodeName) {
				case "#text":
					if(child.nodeValue.trim() != "") {
						result += snst + "/* " + child.nodeValue.trim().replace(/\n/g,snst + "   ") + " */";
					}
					break;
				...
				case "nomatch":
				    continue;
				default:
					result += snst + startTag(child).replace(/\n/g,snst + "   ");
			}
			...
			switch(child.nodeName) {
				...
				case "#text":
				    if(child.nodeValue.trim() != "") {
						result += snst + "__browser__.addText(unescape(\"" + escape(child.nodeValue.trim()) + "\"));";
					}
					break;
				case "assign":
				...
				case "field":
					...
					result += this.parse(child,level + 2);
					result += snst + "\t\tfilled = "  
						+ "__browser__.getInput(\"" + child.getAttribute("name") + "\"," + child.getAttribute("name") + ");" ;
					...
					for(var j=0; j < child.childNodes.length; j++) {
						if(child.childNodes[j].nodeName == "catch" 
							...
							result += snst + "\t\t\t\t" + startTag(child.childNodes[j]);
							...
							result += snst + "\t\t\t\t}";
							result += snst + "\t\t\t\t" + endTag(child.childNodes[j]);
							result += snst + "\t\t\t\telse";
						}
					}
				...
				case "menu": // Menu is equivalent to form don't move this code
					...
					result += snst + "\t\tthis.fields = \"" + fields.trim() + "\";";
					...
					if(child.nodeName == "menu") {
						result += snst + "\t\t\t\t\t\tfilled = __browser__.getInput(\"type your choice\",\"\");";
						result += snst + "\t\t\t\t\t\tif(filled) {";
				...
				case "grammar":
					...
					//*************************************************************
					__browser__.warning(this,"Unsupported element: grammar");
					break;
				case "if":
					result += snst + "if(" + child.getAttribute("cond").replace(/\n/g,snst + "\t") + ") {";
					result += this.parse(child, level + 1);
				...
				case "prompt":
					if(child.getAttribute("timeout")) {
						__browser__.warning(this,"Unsupported attribute: timeout");
					}
					if(child.getAttribute("cond")) {
						result += snst + "if(" + child.getAttribute("cond").replace(/\n/g,snst + "\t") + ") {";
					}	
					...
					break;
				case "property":
					//result += snst + "__browser__.setProperty(\"" + child.getAttribute("name") + "\",\"" + child.getAttribute("value").replace(/\n/g,snst + "\t") + "\");";
					break;
				case "script":
					if(child.firstChild) {
				...
				case "submit":
					...
					result += snst + "return __browser__.getQuery(_get";
					...
					*/
					if(child.getAttribute("method")) {
						__browser__.warning(this,"Unsupported attribute: method");
					}
					...
					*/
					if(child.getAttribute("enctype")) {
						__browser__.warning(this,"Unsupported attribute: enctype");
					}
				...
				case "value":
					if(child.getAttribute("class")) {
						__browser__.warning(this,"Unsupported attribute: class");
					}
					if(child.getAttribute("mode")) {
						__browser__.warning(this,"Unsupported attribute: mode");
					}
					if(child.getAttribute("recSrc")) {
						__browser__.warning(this,"Unsupported attribute: recSrc");
					}
					if(child.getAttribute("expr")) {
						result += snst + "__expr = " + child.getAttribute("expr").replace(/\n/g,snst + "\t") + ";";
						result += snst + "__browser__.addText(__expr);";
					}
					break;
				case "var":
				...
				case "vxml":
					...
					result += snst + "\t\t\t\tif(_newnext[0] == \"\" || _newnext[0] == this.url) {";
